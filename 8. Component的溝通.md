...
`encapsulation` 可以設定要不要具有這種特性，`ViewEncapsulation.Emulated` 是原先的預設值、 `ViewEncapsulation.None` 是不替 Component 添加額外的屬性、`ViewEncapsulation.ShadowDom` 則採用瀏覽器原生提供的 ShadowDom。
